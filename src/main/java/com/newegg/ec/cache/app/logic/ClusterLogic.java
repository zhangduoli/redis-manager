...
public class ClusterLogic {
    ...
    public Object flush(int id){
        Cluster cluster = clusterDao.getCluster(id);
        List<Map<String, String>> nodeList = JedisUtil.getNodeList(cluster.getAddress().split(":")[0],
                Integer.valueOf(cluster.getAddress().split(":")[1]) ,
                true);
        for (Map<String, String> node : nodeList){
            Jedis jedis = null;
            try {
                jedis = new Jedis(node.get("ip"), JedisUtil.getPort(node.get("port")), 30000);
                jedis.flushAll();
            }catch (Exception e){
                jedis.eval("");
            } finally {
