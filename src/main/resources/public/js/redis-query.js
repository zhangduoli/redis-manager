...
$(document).on("click", "#query-db", function () {
    smarty.fopen("/cluster/redisDbList?address=" + window.address, "monitor/redis_query", true, {
        title: "Query",
        width: 800,
        height: 500
    }, function (obj) {
...
$("body").delegate("#flush","click",function () {
    console.log("点击事件,集群id"+window.clusterId)
    redisFlush(window.clusterId, function (obj) {
        var result = obj.res;
        alert(result)
    })
})
